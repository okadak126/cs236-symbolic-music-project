...
21 KB
Storage used
35 KBOwned by Stanford University
Location
...
def get_dataset_custom(dataset='',
                data_shape=(2,),
                problem='vae',
                batch_size=128,
                normalize=True,
  ...
  # CREATE SCALE INPUTS
  train_dataset = tf.data.TFRecordDataset('data/custom_midi/custom_midi_emb_trans/train-0000.tfrecord')
  input_list = []
  for raw_record in train_dataset:
    ...
    for key, feature in example.features.feature.items():
      ...
      if key == "inputs":
        ...
        input_list.append(np.reshape(np.array(getattr(feature, kind).value), (32,512)))
        
  def create_generator(list_of_arrays):
    for i in list_of_arrays:
      yield i
  scale_ds = tf.data.Dataset.from_generator(lambda: create_generator(input_list),output_types= tf.float32, output_shapes=(32,512))

  # Batch.
  train_ds = train_ds.batch(batch_size, drop_remainder=True)
  eval_ds = eval_ds.batch(batch_size, drop_remainder=True)
  scale_ds = scale_ds.batch(batch_size, drop_remainder=True)

  train_ds = train_ds.map(partial(deconstruct_dict, problem=problem),
                          num_parallel_calls=AUTOTUNE)
  eval_ds = eval_ds.map(partial(deconstruct_dict, problem=problem),
                        num_parallel_calls=AUTOTUNE)
#   scale_ds = scale_ds.map(partial(deconstruct_dict, problem=problem),
#                         num_parallel_calls=AUTOTUNE)
  ...
  if problem != 'tokens':
    ...
    eval_ds = eval_ds.map(lambda example: tf.py_function(
        partial(data_transform, problem=problem, pca=pca), [example], tf.float32
                          ...
                          num_parallel_calls=AUTOTUNE)
    scale_ds = scale_ds.map(lambda example: tf.py_function(
        partial(data_transform, problem=problem, pca=pca), [example], tf.float32
    ),
                          num_parallel_calls=AUTOTUNE)
    ...
    eval_ds = eval_ds.map(partial(slice_transform,
                                  ...
                                  dim_weights=dim_weights),
                          num_parallel_calls=AUTOTUNE)
    scale_ds = scale_ds.map(partial(slice_transform,
                                  problem=problem,
                                  slice_idx=slice_idx,
                                  dim_weights=dim_weights),
                          num_parallel_calls=AUTOTUNE)
  ...
  if normalize:
    ...
    scale_min, scale_max = data_utils.compute_dataset_min_max(
        scale_ds,
        ds_split='scale',
        cache=True,
        cache_dir=os.path.expanduser(dataset),
    ...
    eval_ds = eval_ds.map(lambda example: normalize_dataset(
        example, eval_min, eval_max),
                          num_parallel_calls=AUTOTUNE)
    scale_ds = scale_ds.map(lambda example: normalize_dataset(
        example, scale_min, scale_max),
                          num_parallel_calls=AUTOTUNE)

  train_ds = train_ds.prefetch(AUTOTUNE)
  eval_ds = eval_ds.prefetch(AUTOTUNE)
  scale_ds = scale_ds.prefetch(AUTOTUNE)
  eval_ds = eval_ds.cache()
  scale_ds = scale_ds.cache()
  
  setattr(train_ds, 'min', train_min)
  setattr(train_ds, 'max', train_max)
  setattr(eval_ds, 'min', eval_min)
  setattr(eval_ds, 'max', eval_max)
  setattr(scale_ds, 'min', scale_min)
  setattr(scale_ds, 'max', scale_max)
...
def get_dataset_scale(dataset='',
  ...
  # CREATE SCALE INPUTS
  train_dataset = tf.data.TFRecordDataset('data/infill_scale/infill_scale_emb_trans/train-0000.tfrecord')
  input_list = []
  for raw_record in train_dataset:
    example = tf.train.Example()
    example.ParseFromString(raw_record.numpy())
...
#                         num_parallel_calls=AUTOTUNE)
  ...
  return train_ds, eval_ds, scale_ds